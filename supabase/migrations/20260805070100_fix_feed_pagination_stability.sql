-- Drop the existing get_edge_feed function
DROP FUNCTION IF EXISTS public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[]);

-- Recreate with p_session_time for stable pagination and remove seen-video demotion
...
BEGIN
  ...
  WHERE
    ...
    AND (v.suppress_until IS NULL OR v.suppress_until < p_session_time)
  ORDER BY
    ...
    -- USING p_session_time TO FIX THE PAGINATION SHIFTING BUG
    (
      COALESCE(v.engagement_score, 0.0) *
      (0.8 + 0.4 * (abs(hashtext(v.id::text || p_seed))::numeric / 2147483647.0)) *
      COALESCE(1.5 / (1.0 + LN(1.0 + EXTRACT(EPOCH FROM GREATEST(p_session_time - v.created_at, interval '0 seconds'))/86400.0)), 1.0) *
      CASE WHEN (
...
-- Secure the RPC
REVOKE EXECUTE ON FUNCTION public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[], TIMESTAMPTZ) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[], TIMESTAMPTZ) FROM authenticated;
REVOKE EXECUTE ON FUNCTION public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[], TIMESTAMPTZ) FROM anon;

GRANT EXECUTE ON FUNCTION public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[], TIMESTAMPTZ) TO service_role;
GRANT EXECUTE ON FUNCTION public.get_edge_feed(UUID, INT, INT, TEXT, TEXT[], TIMESTAMPTZ) TO postgres;
