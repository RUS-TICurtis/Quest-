...
class EventsState {
  ...
  EventsState({required this.events, this.selectedFilter = 'All'});
  ...
  List<Event> get filteredEvents {
    ...
    return events
        .where((e) => e.category.toLowerCase() == selectedFilter.toLowerCase())
        .toList();
  }

  EventsState copyWith({List<Event>? events, String? selectedFilter}) {
    return EventsState(
...
class EventsNotifier extends AsyncNotifier<EventsState> {
  ...
  Future<EventsState> build() async {
    ...
    final updatedEvents = events
        .map((e) => e.copyWith(isRsvpd: rsvpdIds.contains(e.id)))
        .toList();
  ...
  Future<void> toggleRsvp(String eventId) async {
    ...
    final updatedEvent = event.copyWith(
      attendeesCount: isCurrentlyRsvpd
  ...
  Future<void> addEvent(Event newEvent) async {
    ...
    state = AsyncData(
      currentState.copyWith(events: [newEvent, ...currentState.events]),
    );
  }
